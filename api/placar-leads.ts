 ...
 * Retorna { leadsUtm, leadsClint, clintAtivo, leadsDistribuicao }
 */
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const sql = `
      ...
      ORDER BY leads DESC
    `
    const [bqResult, leadsClint] = await Promise.all([
      bqQuery(sql, params),
    ...
    res.json({ month, leadsUtm, leadsClint, clintAtivo: Object.keys(leadsClint).length > 0, leadsDistribuicao })
  } catch (err) {
